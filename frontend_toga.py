...
# Disable warnings for demo environments (self-signed etc.)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
ssl._create_default_https_context = ssl._create_unverified_context
warnings.filterwarnings("ignore", category=DeprecationWarning)
...
class QuranApp(toga.App):
    ...
    # ============ Boot ============
    def startup(self):
        self.main_window = toga.MainWindow(title=self.formal_name, size=(520, 820))
        self.show_login_screen()
        self.main_window.show()
    ...
    def show_login_screen(self):
        ...
        logo_label = toga.Label(APP_LOGO, style=Pack(text_align="center", font_size=40, padding_bottom=10))
        title_label = toga.Label("📖 Quran App", style=Pack(text_align="center", font_size=24, font_weight="bold", padding_bottom=20))
        ...
        self.username_input = toga.TextInput(placeholder="Username", style=Pack(padding=12, flex=1, padding_bottom=10))
        self.password_input = toga.PasswordInput(placeholder="Password", style=Pack(padding=12, flex=1, padding_bottom=20))

        login_btn = toga.Button("🔐 Login", on_press=self.login_handler, style=Pack(padding=15, background_color=THEME_COLORS["primary"], color="white", padding_bottom=10))
        register_student_btn = toga.Button("🎓 Register Student", on_press=self.go_to_register_student, style=Pack(padding=15, background_color=THEME_COLORS["secondary"], color="white", padding_bottom=10))
        register_teacher_btn = toga.Button("👨‍🏫 Register Teacher", on_press=self.go_to_register_teacher, style=Pack(padding=15, background_color=THEME_COLORS["accent"], color="white"))
        test_btn = toga.Button("📡 Test Connection", on_press=self.test_connection, style=Pack(padding=10, background_color=THEME_COLORS["info"], color="white", padding_top=20))

        for w in (logo_label, title_label, self.connection_status, self.username_input, self.password_input, login_btn, register_student_btn, register_teacher_btn, test_btn):
            main_box.add(w)
    ...
    def show_dashboard_screen(self):
        ...
        display_name = self.user.get("full_name") or self.user.get("username") or "User"
        title_label = toga.Label(f"🎉 Welcome, {display_name}!", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))
        role_label = toga.Label(f"Role: {self.user.get('role','-')}", style=Pack(text_align="center", font_size=14, padding_bottom=30, color=THEME_COLORS["info"]))
        ...
        for text, handler, color in menu_items:
            if self.user.get("role") != "teacher" and text == "📚 Create Class":
                continue
            btn = toga.Button(text, on_press=handler, style=Pack(padding=15, background_color=color, color="white", padding_bottom=8))
            menu_box.add(btn)

        for w in (title_label, role_label, menu_box):
            main_box.add(w)
    ...
    def show_register_student_screen(self):
        ...
        title_label = toga.Label("🎓 Register Student", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))

        self.student_fullname = toga.TextInput(placeholder="Full Name *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.student_email = toga.TextInput(placeholder="Email *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.student_password = toga.PasswordInput(placeholder="Password *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.student_grade = toga.Selection(items=["Beginner", "Intermediate", "Advanced", "Expert"], style=Pack(padding=10, flex=1, padding_bottom=20))

        register_btn = toga.Button("✅ Register", on_press=self.register_student_handler, style=Pack(padding=15, background_color=THEME_COLORS["success"], color="white", padding_bottom=10))
        back_btn = toga.Button("↩️ Back", on_press=self.show_login_screen, style=Pack(padding=15, background_color=THEME_COLORS["danger"], color="white"))

        for w in (title_label, self.student_fullname, self.student_email, self.student_password, self.student_grade, register_btn, back_btn):
            main_box.add(w)
    ...
    def show_register_teacher_screen(self):
        ...
        title_label = toga.Label("👨‍🏫 Register Teacher", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))

        self.teacher_username = toga.TextInput(placeholder="Username *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.teacher_password = toga.PasswordInput(placeholder="Password *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.teacher_email = toga.TextInput(placeholder="Email", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.teacher_fullname = toga.TextInput(placeholder="Full Name *", style=Pack(padding=10, flex=1, padding_bottom=10))
        self.teacher_specialty = toga.Selection(items=["Tajweed", "Recitation", "Tafsir", "Fiqh", "Arabic", "Islamic Studies"], style=Pack(padding=10, flex=1, padding_bottom=20))

        register_btn = toga.Button("✅ Register", on_press=self.register_teacher_handler, style=Pack(padding=15, background_color=THEME_COLORS["success"], color="white", padding_bottom=10))
        back_btn = toga.Button("↩️ Back", on_press=self.show_login_screen, style=Pack(padding=15, background_color=THEME_COLORS["danger"], color="white"))

        for w in (title_label, self.teacher_username, self.teacher_password, self.teacher_email, self.teacher_fullname, self.teacher_specialty, register_btn, back_btn):
            main_box.add(w)
    ...
    def show_quran(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("📖 Holy Quran", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))
        ...
        scroll_box.content = surahs_box
        back_btn = toga.Button("↩️ Back", on_press=self.show_dashboard_screen, style=Pack(padding=15, background_color=THEME_COLORS["danger"], color="white", padding_top=20))

        for w in (title_label, scroll_box, back_btn):
            main_box.add(w)
    ...
    def show_surah(self, surah_id):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label(f"📖 Surah {surah_id}", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))
        content_label = toga.Label(
            "Surah content will be displayed here...\n\n"
            "Bismillahir Rahmanir Rahim\n"
            "Alhamdu lillahi rabbil 'alamin\n"
        ...
        back_btn = toga.Button("↩️ Back", on_press=self.show_quran, style=Pack(padding=15, background_color=THEME_COLORS["danger"], color="white", padding_top=20))

        for w in (title_label, content_label, back_btn):
            main_box.add(w)
    ...
    def show_my_classes(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("🎓 My Classes", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))
        ...
        for cls in classes:
            card = toga.Box(style=Pack(direction=COLUMN, padding=15, background_color="#f5f5f5", padding_bottom=10))
            card.add(toga.Label(cls["title"], style=Pack(font_size=16, font_weight="bold", padding_bottom=5)))
            card.add(toga.Label(f"👨‍🏫 {cls['teacher']}", style=Pack(font_size=14, padding_bottom=3)))
            card.add(toga.Label(f"⏰ {cls['time']}", style=Pack(font_size=12, color="gray")))
            join_btn = toga.Button("🎧 Join Class", on_press=lambda w, c=cls: self.join_class(c["id"]), style=Pack(padding=10, background_color=THEME_COLORS["success"], color="white", padding_top=10))
            card.add(join_btn)
            classes_box.add(card)
    ...
    def show_create_class(self, widget=None):
        # Backend endpoint for classes isn't implemented in your backend.py yet.
        self.show_info("Not Implemented", "The 'Create Class' feature is not implemented on the server yet.")
        return

    def show_chat(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("💬 Class Chat", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))

        classes = [{"id": 1, "name": "Basic Tajweed"}, {"id": 2, "name": "Quran Recitation"}, {"id": 3, "name": "Quran Tafsir"}]

        scroll_box = toga.ScrollContainer(style=Pack(flex=1))
        chat_box = toga.Box(style=Pack(direction=COLUMN, padding=10))

        for cls in classes:
            chat_btn = toga.Button(f"💬 {cls['name']}", on_press=lambda w, c=cls: self.open_chat_room(c["id"]), style=Pack(padding=12, background_color=THEME_COLORS["info"], color="white", padding_bottom=8))
            chat_box.add(chat_btn)
    ...
    def show_progress(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("📊 Learning Progress", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))
        ...
        for data in progress_data:
            card = toga.Box(style=Pack(direction=COLUMN, padding=15, background_color="#f5f5f5", padding_bottom=10))
            card.add(toga.Label(data["subject"], style=Pack(font_size=16, font_weight="bold", padding_bottom=5)))
            card.add(toga.Label(f"📈 Progress: {data['progress']}%", style=Pack(font_size=14, padding_bottom=3)))
            card.add(toga.Label(f"🎯 Grade: {data['grade']}", style=Pack(font_size=14, color=THEME_COLORS["success"])))
            progress_box.add(card)
    ...
    def show_profile(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("👤 My Profile", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))

        profile_box = toga.Box(style=Pack(direction=COLUMN, padding=20, background_color="#f5f5f5"))

        info_items = [
            ("👤 Full Name", self.user.get("full_name") or "-"),
            ("📧 Email", self.user.get("email") or "-"),
            ("🎯 Role", self.user.get("role") or "-"),
            ("📊 Level", self.user.get("grade") or "-"),
            ("🏆 Specialty", self.user.get("specialty") or "-"),
            ("✅ Approved", "Yes" if self.user.get("approved") else "No"),
        ]

        for label, value in info_items:
            row = toga.Box(style=Pack(direction=ROW, padding_bottom=10))
            row.add(toga.Label(f"{label}: {value}", style=Pack(flex=1, font_size=14)))
            profile_box.add(row)

        edit_btn = toga.Button("✏️ Edit Profile", on_press=self.edit_profile, style=Pack(padding=15, background_color=THEME_COLORS["warning"], color="white", padding_top=20))
        back_btn = toga.Button("↩️ Back", on_press=self.show_dashboard_screen, style=Pack(padding=15, background_color=THEME_COLORS["danger"], color="white", padding_top=10))

        for w in (title_label, profile_box, edit_btn, back_btn):
            main_box.add(w)
    ...
    def show_payment(self, widget=None):
        main_box = toga.Box(style=Pack(direction=COLUMN, padding=20, flex=1))
        title_label = toga.Label("💰 Payment & Purchase", style=Pack(text_align="center", font_size=20, font_weight="bold", padding_bottom=20))

        classes = [{"id": 1, "title": "Basic Tajweed", "price": 10.0}, {"id": 2, "title": "Quran Recitation", "price": 15.0}, {"id": 3, "title": "Quran Tafsir", "price": 20.0}]
        ...
        for cls in classes:
            card = toga.Box(style=Pack(direction=COLUMN, padding=15, background_color="#f5f5f5", padding_bottom=10))
            card.add(toga.Label(cls["title"], style=Pack(font_size=16, font_weight="bold", padding_bottom=5)))
            card.add(toga.Label(f"💰 Price: {cls['price']} USDT", style=Pack(font_size=14, padding_bottom=10)))
            buy_btn = toga.Button("💳 Buy Class", on_press=lambda w, c=cls: self.make_payment(c["id"], c["price"]), style=Pack(padding=10, background_color=THEME_COLORS["success"], color="white"))
            card.add(buy_btn)
            payment_box.add(card)
    ...
    async def login(self):
        ...
        try:
            payload = {"username": username, "password": password}
            resp = requests.post(f"{BASE_URL}/login", json=payload, verify=False, timeout=30)

            if resp.status_code == 200:
                data = resp.json()
                # backend returns: { success: True, user: {...} }
                if data.get("success") and data.get("user"):
                    compact_user = data["user"]
                    # fetch full profile (grade, specialty, approved)
                    user_id = compact_user.get("id")
                    full = requests.get(f"{BASE_URL}/user/{user_id}", verify=False, timeout=30)
                    self.user = full.json() if full.status_code == 200 else compact_user
                    self.show_info("Success", "Login successful!")
                    self.show_dashboard_screen()
                else:
                    self.show_error("Error", "Invalid credentials")
            else:
                self.show_error("Error", f"Login failed ({resp.status_code})")
        except Exception as e:
            self.show_error("Error", f"Server connection error: {str(e)}")

    async def register_student(self):
        fullname = (self.student_fullname.value or "").strip()
        email = (self.student_email.value or "").strip()
        password = (self.student_password.value or "").strip()
        level = self.student_grade.value

        if not all([fullname, email, password, level]):
            self.show_error("Error", "Please fill all required fields")
            return

        try:
            # backend expects: {name, email, password, level}
            payload = {"name": fullname, "email": email, "password": password, "level": level}
            resp = requests.post(f"{BASE_URL}/register/student", json=payload, verify=False, timeout=30)

            if resp.status_code == 200:
                self.show_info("Success", "Student registration successful!")
                self.show_login_screen()
            else:
                msg = resp.json().get("detail", f"Registration failed ({resp.status_code})")
                self.show_error("Error", msg)
        except Exception as e:
            self.show_error("Error", f"Server connection error: {str(e)}")

    async def register_teacher(self):
        username = (self.teacher_username.value or "").strip()
        password = (self.teacher_password.value or "").strip()
        email = (self.teacher_email.value or "").strip()
        fullname = (self.teacher_fullname.value or "").strip()
        specialty = self.teacher_specialty.value
        ...
        try:
            ...
            resp = requests.post(f"{BASE_URL}/register/teacher", json=payload, verify=False, timeout=30)
    ...
    async def test_server_connection(self):
        try:
            ...
            if resp.status_code == 200 and resp.json().get("status") == "healthy":
                self.show_info("Connection Successful", "Server is healthy and database is connected.")
            else:
                self.show_error("Error", f"Health check failed ({resp.status_code})")
        except Exception as e:
            self.show_error("Error", f"Cannot connect to server: {str(e)}")
